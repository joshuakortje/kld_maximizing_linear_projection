...
print('SVC with CIFAR-10 based data')
training_p = 5000
test_p = 1000
r = 2
trim = 4
...
cifar10_train_data, cifar10_test_data = get_cifar10_2_class(trim)

class1_mean, class1_cov = calculate_statistics(cifar10_train_data[0])
class2_mean, class2_cov = calculate_statistics(cifar10_train_data[1])
measured_means = [class1_mean, class2_mean]
measured_covs = [class1_cov, class2_cov]

# Generate actually gaussian samples from the measured distributions
train_data = gen_gaussian_samples(measured_means, measured_covs, training_p)
test_data = gen_gaussian_samples(measured_means, measured_covs, test_p)
...
class_1_data = test_data[0]
class_2_data = test_data[1]
